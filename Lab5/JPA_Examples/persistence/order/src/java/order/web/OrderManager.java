...
public class OrderManager {
    ...
    public void addLineItem() {
        try {
            ...
            logger.info(
                    "Add: there are " + lineItems.size() + " line items in " + currentOrder + ".");
            request.addLineItem(
                    this.currentOrder,
    ...
    public void deleteLineItem() {
        try {
            List<LineItem> lineItems = request.getLineItems(currentOrder);
            logger.info(
                    "Delete: there are " + lineItems.size() + " line items in " + currentOrder + ".");
            request.deleteLineItem(
                    this.currentOrder,
                    this.selectedPartNumber,
                    this.selectedPartRevision,
                    1);
            logger.info("Deleting line item " + this.selectedPartNumber + " from order # " + this.currentOrder);

            //this.clearSelected();
        } catch (Exception e) {
            logger.warning(
                    "Problem deleting line item " + this.selectedPartNumber + " from order ID " + this.currentOrder);
        }
    }
