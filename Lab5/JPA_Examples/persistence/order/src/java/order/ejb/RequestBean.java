...
public class RequestBean {
    ...
    public void addLineItem(
        ...
        try {
            ...
            logger.warning("Couldn't add " + partNumber + " to order ID " + orderId+ ".");
            throw new EJBException(e.getMessage());
        }
    }
    ...
    public void deleteLineItem(
        ...
        try {
            ...
            for (Iterator iterator = lineItems.iterator(); iterator.hasNext();) {
                ...
                if (lineItem.getVendorPart().getPart().getPartNumber().equals(partNumber)) {
                    int qty = lineItem.getQuantity();
                    if (qty > 1) {
                        lineItem.setQuantity(qty-1);
                    }
                    else {
                        order.deleteLineItem((LineItem) lineItem);
                        lineItems.remove((LineItem) lineItem);
                        break;
                    }
                }
            }
            ...
            logger.warning("Couldn't delete " + partNumber + " from order ID " + orderId + ".");
            throw new EJBException(e.getMessage());
        }
    }
